...
void main()
{
  ...
  yuv.rgba = vec4(texture(m_sampY, offsetY).r, texture(m_sampU, offsetU).r, texture(m_sampV, offsetV).r, 1.0);
  rgb   = m_yuvmat * yuv;
  rgb.a = m_alpha;

  yuvBelow.rgba = vec4(texture(m_sampY, belowY).r, texture(m_sampU, belowU).r, texture(m_sampV, belowV).r, 1.0);
