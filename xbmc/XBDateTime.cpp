...
std::string CDateTime::GetAsDBDateTime() const
{
  ...
  return StringUtils::Format("%04i-%02i-%02i %02i:%02i:%02i", st.year, st.month, st.day, st.hour, st.minute, st.second);
}
...
std::string CDateTime::GetAsSaveString() const
{
  ...
  return StringUtils::Format("%04i%02i%02i_%02i%02i%02i", st.year, st.month, st.day, st.hour, st.minute, st.second);
}
...
std::string CDateTime::GetAsLocalizedTime(const std::string &format, bool withSeconds) const
{
  ...
  const std::string& strMeridiem = CLangInfo::MeridiemSymbolToString(dateTime.hour > 11 ? MeridiemSymbolPM : MeridiemSymbolAM);
...
std::string CDateTime::GetAsW3CDateTime(bool asUtc /* = false */) const
{
  ...
  std::string result = StringUtils::Format("%04i-%02i-%02iT%02i:%02i:%02i", st.year, st.month, st.day, st.hour, st.minute, st.second);
  if (asUtc)
    return result + "Z";
