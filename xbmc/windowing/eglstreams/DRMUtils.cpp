...
// void CDRMUtils::DrmFbDestroyCallback(void *data)
// {
...
uint32_t CDRMUtils::CreateFb()
{
  ...
  createRequest.width = m_mode->hdisplay;
  createRequest.height = m_mode->vdisplay;
  createRequest.bpp = 32;

  auto ret = drmIoctl(m_fd, DRM_IOCTL_MODE_CREATE_DUMB, &createRequest);
  if (ret < 0)
  {
    CLog::Log(LOGDEBUG, "CDRMAtomic::%s - Unable to create dumb buffer", __FUNCTION__);
    return -1;
  }

  ret = drmModeAddFB(m_fd, m_mode->hdisplay, m_mode->vdisplay, 24, 32, createRequest.pitch, createRequest.handle, &fb);
  if (ret)
  {
    CLog::Log(LOGDEBUG, "CDRMAtomic::%s - Unable to add fb", __FUNCTION__);
    return -1;
  }

  mapRequest.handle = createRequest.handle;

  ret = drmIoctl(m_fd, DRM_IOCTL_MODE_MAP_DUMB, &mapRequest);
  if (ret)
  {
    CLog::Log(LOGDEBUG, "CDRMAtomic::%s - Unable to map dumb buffer", __FUNCTION__);
    return -1;
  }

  map = static_cast<uint8_t*>(mmap(0, createRequest.size, PROT_READ | PROT_WRITE, MAP_SHARED, m_fd, mapRequest.offset));
  if (map == MAP_FAILED)
  {
    CLog::Log(LOGDEBUG, "CDRMAtomic::%s - Failed to mmap(2) fb", __FUNCTION__);
    return -1;
  }
