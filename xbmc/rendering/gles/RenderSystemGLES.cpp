...
{
  m_pShader[SM_DEFAULT] = new CGLESShader("gles_guishader_vert.vert", "gles_guishader_frag_default.frag");
  if (!m_pShader[SM_DEFAULT]->CompileAndLink())
  {
    m_pShader[SM_DEFAULT]->Free();
    delete m_pShader[SM_DEFAULT];
    m_pShader[SM_DEFAULT] = nullptr;
    CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_default.frag - compile and link failed");
  }

  m_pShader[SM_TEXTURE] = new CGLESShader("gles_guishader_frag_texture.frag");
  if (!m_pShader[SM_TEXTURE]->CompileAndLink())
  {
    m_pShader[SM_TEXTURE]->Free();
    delete m_pShader[SM_TEXTURE];
    m_pShader[SM_TEXTURE] = nullptr;
    CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_texture.frag - compile and link failed");
  }

  m_pShader[SM_MULTI] = new CGLESShader("gles_guishader_frag_multi.frag");
  if (!m_pShader[SM_MULTI]->CompileAndLink())
  {
    m_pShader[SM_MULTI]->Free();
    delete m_pShader[SM_MULTI];
    m_pShader[SM_MULTI] = nullptr;
    CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_multi.frag - compile and link failed");
  }

  m_pShader[SM_FONTS] = new CGLESShader("gles_guishader_frag_fonts.frag");
  if (!m_pShader[SM_FONTS]->CompileAndLink())
  {
    m_pShader[SM_FONTS]->Free();
    delete m_pShader[SM_FONTS];
    m_pShader[SM_FONTS] = nullptr;
    CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_fonts.frag - compile and link failed");
  }

  m_pShader[SM_TEXTURE_NOBLEND] = new CGLESShader("gles_guishader_frag_texture_noblend.frag");
  if (!m_pShader[SM_TEXTURE_NOBLEND]->CompileAndLink())
  {
    ...
    CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_texture_noblend.frag - compile and link failed");
  }

  m_pShader[SM_MULTI_BLENDCOLOR] = new CGLESShader("gles_guishader_frag_multi_blendcolor.frag");
  if (!m_pShader[SM_MULTI_BLENDCOLOR]->CompileAndLink())
  {
    ...
    CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_multi_blendcolor.frag - compile and link failed");
  }

  m_pShader[SM_TEXTURE_RGBA] = new CGLESShader("gles_guishader_frag_rgba.frag");
  if (!m_pShader[SM_TEXTURE_RGBA]->CompileAndLink())
  {
    ...
    CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_rgba.frag - compile and link failed");
  }

  m_pShader[SM_TEXTURE_RGBA_BLENDCOLOR] = new CGLESShader("gles_guishader_frag_rgba_blendcolor.frag");
  if (!m_pShader[SM_TEXTURE_RGBA_BLENDCOLOR]->CompileAndLink())
  {
    ...
    CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_rgba_blendcolor.frag - compile and link failed");
  }

  m_pShader[SM_TEXTURE_RGBA_BOB] = new CGLESShader("gles_guishader_frag_rgba_bob.frag");
  if (!m_pShader[SM_TEXTURE_RGBA_BOB]->CompileAndLink())
  {
    ...
    CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_rgba_bob.frag - compile and link failed");
  }

  if (!IsExtSupported("GL_OES_EGL_image_external"))
  {
    m_pShader[SM_TEXTURE_RGBA_OES] = new CGLESShader("gles_guishader_frag_rgba_oes.frag");
    if (!m_pShader[SM_TEXTURE_RGBA_OES]->CompileAndLink())
    {
      ...
      CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_rgba_oes.frag - compile and link failed");
    }


    m_pShader[SM_TEXTURE_RGBA_BOB_OES] = new CGLESShader("gles_guishader_frag_rgba_bob_oes.frag");
    if (!m_pShader[SM_TEXTURE_RGBA_BOB_OES]->CompileAndLink())
    {
      ...
      CLog::Log(LOGERROR, "GUI Shader gles_guishader_frag_rgba_bob_oes.frag - compile and link failed");
    }
  }
  else
  {
