...
CGUIProgressControl::CGUIProgressControl(int parentID, int controlID,
    ...
    , m_guiLeft(CGUITexture::GetTexture(posX, posY, width, height, leftTexture))
    , m_guiMid(CGUITexture::GetTexture(posX, posY, width, height, midTexture))
    , m_guiRight(CGUITexture::GetTexture(posX, posY, width, height, rightTexture))
    , m_guiOverlay(CGUITexture::GetTexture(posX, posY, width, height, overlayTexture))
{
  m_fPercent = 0;
  m_iInfoCode = 0;
...
CGUIProgressControl::CGUIProgressControl(const CGUIProgressControl &left)
  : CGUIControl(left),
    m_guiBackground(CGUITexture::GetTexture(*left.m_guiBackground)),
    m_guiLeft(CGUITexture::GetTexture(*left.m_guiLeft)),
    m_guiMid(CGUITexture::GetTexture(*left.m_guiMid)),
    m_guiRight(CGUITexture::GetTexture(*left.m_guiRight)),
    m_guiOverlay(CGUITexture::GetTexture(*left.m_guiOverlay))
{
  m_fPercent = 0;
  m_iInfoCode = 0;
  ControlType = GUICONTROL_PROGRESS;
  m_bReveal = left.m_bReveal;
  m_bChanged = false;
}
