...
void CGUIFontTTFGLES::LastEnd()
{
  ...
  if (!m_vertex.empty())
  {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(SVertex) * vecVertices.size(), vecVertices.data(), GL_STATIC_DRAW);

    glVertexAttribPointer(posLoc, 3, GL_FLOAT, GL_FALSE, sizeof(SVertex), reinterpret_cast<const GLvoid*>(offsetof(SVertex, x)));
    glVertexAttribPointer(colLoc, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(SVertex), reinterpret_cast<const GLvoid*>(offsetof(SVertex, r)));
    glVertexAttribPointer(tex0Loc, 2, GL_FLOAT, GL_FALSE, sizeof(SVertex), reinterpret_cast<const GLvoid*>(offsetof(SVertex, u)));
  ...
  if (!m_vertexTrans.empty())
  {
    ...
    for (size_t i = 0; i < m_vertexTrans.size(); i++)
    {
      ...
      for (size_t character = 0; m_vertexTrans[i].vertexBuffer->size > character; character += 1000)
      {
        ...
        glVertexAttribPointer(posLoc, 3, GL_FLOAT, GL_FALSE, sizeof(SVertex), reinterpret_cast<const GLvoid*>(reinterpret_cast<char*>(character*sizeof(SVertex)*4) + offsetof(SVertex, x)));
        glVertexAttribPointer(colLoc, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(SVertex), reinterpret_cast<const GLvoid*>(reinterpret_cast<char*>(character*sizeof(SVertex)*4) + offsetof(SVertex, r)));
        glVertexAttribPointer(tex0Loc, 2, GL_FLOAT, GL_FALSE, sizeof(SVertex), reinterpret_cast<const GLvoid*>(reinterpret_cast<char*>(character*sizeof(SVertex)*4) + offsetof(SVertex, u)));
