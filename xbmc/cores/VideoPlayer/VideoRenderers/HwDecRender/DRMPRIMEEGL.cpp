  ...
  if (descriptor && descriptor->nb_layers)
  {
    ...
    switch (layer->format)
    {
      case DRM_FORMAT_NV12:
      {
        ...
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_LINUX_DRM_FOURCC_EXT: %i", __FUNCTION__, fourcc_code('R', '8', ' ', ' '));
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_WIDTH,: %i", __FUNCTION__, buffer->GetWidth());
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_HEIGHT: %i", __FUNCTION__, buffer->GetHeight());
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_DMA_BUF_PLANE0_FD_EXT: %i", __FUNCTION__, descriptor->objects[layer->planes[0].object_index].fd);
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_DMA_BUF_PLANE0_OFFSET_EXT: %i", __FUNCTION__, layer->planes[0].offset);
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_DMA_BUF_PLANE0_PITCH_EXT: %i", __FUNCTION__, layer->planes[0].pitch);
        ...
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_LINUX_DRM_FOURCC_EXT: %i", __FUNCTION__, fourcc_code('G', 'R', '8', '8'));
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_WIDTH,: %i", __FUNCTION__, (buffer->GetWidth() + 1) >> 1);
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_HEIGHT: %i", __FUNCTION__, (buffer->GetHeight() + 1) >> 1);
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_DMA_BUF_PLANE0_FD_EXT: %i", __FUNCTION__, descriptor->objects[layer->planes[1].object_index].fd);
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_DMA_BUF_PLANE0_OFFSET_EXT: %i", __FUNCTION__, layer->planes[1].offset);
        CLog::Log(LOGDEBUG, "CRendererDRMPRIMEGLES::%s - EGL_DMA_BUF_PLANE0_PITCH_EXT: %i", __FUNCTION__, layer->planes[1].pitch);
        ...
        m_interop.glEGLImageTargetTexture2DOES(m_interop.textureTarget, eglImageY);
        ...
        glTexParameteri(m_interop.textureTarget, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(m_interop.textureTarget, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        m_interop.glEGLImageTargetTexture2DOES(m_interop.textureTarget, eglImageVU);
        glGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_FORMAT, &format);
