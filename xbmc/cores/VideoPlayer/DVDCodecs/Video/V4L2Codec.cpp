...
{
  if (!CV4L2::MmapBuffers(m_fd, V4L2_OUTPUT_BUFFERS_COUNT, &m_OutputBuffers, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE))
  {
  ...
  auto ret = m_fd->streamon(V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
  if (ret < 0)
  {
...
bool V4L2Codec::QueueOutputBuffer(int index, uint8_t* demuxer_content, int demuxer_bytes, double pts)
{
  if (demuxer_bytes < m_OutputBuffers.g_length(0))
  {
    //memcpy((uint8_t *)m_OutputBuffers.g_mmapping(index, 0), demuxer_content, demuxer_bytes);
    m_OutputBuffers.s_mmapping(index, 0, demuxer_content);
