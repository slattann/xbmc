...
bool CVideoPlayer::OpenFile(const CFileItem& file, const CPlayerOptions &options)
{
  ...
  if (IsRunning())
  {
    ...
    m_messenger.Put(std::shared_ptr<CDVDMsgOpenFile>(new CDVDMsgOpenFile(params)), 1);
...
void CVideoPlayer::Process()
{
  ...
  while (!m_bAbortRequest)
  {
    ...
    if (!pPacket)
    {
      ...
      if (m_CurrentVideo.inited)
      {
        m_VideoPlayerVideo->SendMessage(std::shared_ptr<CDVDMsg>(new CDVDMsg(CDVDMsg::VIDEO_DRAIN)));
      }

      { // TODO: only used by OMXPlayer, find better solution
        if (m_CurrentAudio.inited)
          m_VideoPlayerAudio->SendMessage(std::shared_ptr<CDVDMsg>(new CDVDMsg(CDVDMsg::GENERAL_EOF)));
        if (m_CurrentVideo.inited)
          m_VideoPlayerVideo->SendMessage(std::shared_ptr<CDVDMsg>(new CDVDMsg(CDVDMsg::GENERAL_EOF)));
      }
...
void CVideoPlayer::ProcessAudioData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
  ...
  m_VideoPlayerAudio->SendMessage(std::shared_ptr<CDVDMsgDemuxerPacket>(new CDVDMsgDemuxerPacket(pPacket, drop)));
...
void CVideoPlayer::ProcessVideoData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
  ...
  m_VideoPlayerVideo->SendMessage(std::shared_ptr<CDVDMsgDemuxerPacket>(new CDVDMsgDemuxerPacket(pPacket, drop)));
...
void CVideoPlayer::ProcessSubData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
  ...
  m_VideoPlayerSubtitle->SendMessage(std::shared_ptr<CDVDMsgDemuxerPacket>(new CDVDMsgDemuxerPacket(pPacket, drop)));
...
void CVideoPlayer::ProcessTeletextData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
  ...
  m_VideoPlayerTeletext->SendMessage(std::shared_ptr<CDVDMsgDemuxerPacket>(new CDVDMsgDemuxerPacket(pPacket, drop)));
}
...
void CVideoPlayer::ProcessRadioRDSData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
  ...
  m_VideoPlayerRadioRDS->SendMessage(std::shared_ptr<CDVDMsgDemuxerPacket>(new CDVDMsgDemuxerPacket(pPacket, drop)));
}
...
void CVideoPlayer::HandlePlaySpeed()
{
          ...
          m_syncTimer.IsTimePast())
      {
        ...
        else
        {
                   ...
                   m_VideoPlayerAudio->GetLevel() == 0)
          {
            ...
            mode.accurate = true;
            mode.sync = true;
            m_messenger.Put(std::shared_ptr<CDVDMsgPlayerSeek>(new CDVDMsgPlayerSeek(mode)));
          }
        }
      }
      ...
      (m_CurrentAudio.syncState == IDVDStreamPlayer::SYNC_WAITSYNC))
  {
         ...
         m_CurrentVideo.syncState == IDVDStreamPlayer::SYNC_INSYNC))
    {
      m_CurrentAudio.syncState = IDVDStreamPlayer::SYNC_INSYNC;
      m_CurrentAudio.avsync = CCurrentStream::AV_SYNC_NONE;
      m_VideoPlayerAudio->SendMessage(std::shared_ptr<CDVDMsgDouble>(new CDVDMsgDouble(CDVDMsg::GENERAL_RESYNC, m_clock.GetClock())), 1);
    }
             ...
             m_CurrentVideo.avsync == CCurrentStream::AV_SYNC_CONT)
    {
      m_CurrentVideo.syncState = IDVDStreamPlayer::SYNC_INSYNC;
      m_CurrentVideo.avsync = CCurrentStream::AV_SYNC_NONE;
      m_VideoPlayerVideo->SendMessage(std::shared_ptr<CDVDMsgDouble>(new CDVDMsgDouble(CDVDMsg::GENERAL_RESYNC, m_clock.GetClock())), 1);
    }
    else if (video && audio)
    {
      double clock = 0;
      ...
      m_CurrentVideo.syncState = IDVDStreamPlayer::SYNC_INSYNC;
      m_CurrentVideo.avsync = CCurrentStream::AV_SYNC_NONE;
      m_VideoPlayerAudio->SendMessage(std::shared_ptr<CDVDMsgDouble>(new CDVDMsgDouble(CDVDMsg::GENERAL_RESYNC, clock)), 1);
      m_VideoPlayerVideo->SendMessage(std::shared_ptr<CDVDMsgDouble>(new CDVDMsgDouble(CDVDMsg::GENERAL_RESYNC, clock)), 1);
      SetCaching(CACHESTATE_DONE);
      UpdatePlayState(0);
...
void CVideoPlayer::SynchronizeDemuxer()
{
  ...
  std::shared_ptr<CDVDMsgGeneralSynchronize> message(new CDVDMsgGeneralSynchronize(500, SYNCSOURCE_PLAYER));
...
void CVideoPlayer::HandleMessages()
{
  ...
  while (m_messenger.Get(pMsg, 0) == MSGQ_OK)
  {
    ...
    {
      std::shared_ptr<CDVDMsgOpenFile> msg = std::static_pointer_cast<CDVDMsgOpenFile>(pMsg);
    ...
    {
      std::shared_ptr<CDVDMsgPlayerSeek> msg = std::static_pointer_cast<CDVDMsgPlayerSeek>(pMsg);
             ...
             m_messenger.GetPacketCount(CDVDMsg::PLAYER_SEEK_CHAPTER) == 0)
    {
      ...
      std::shared_ptr<CDVDMsgPlayerSeekChapter> msg = std::static_pointer_cast<CDVDMsgPlayerSeekChapter>(pMsg);
      double start = DVD_NOPTS_VALUE;
      int offset = 0;
    ...
    {
      std::shared_ptr<CDVDMsgPlayerSetAudioStream> pMsg2 = std::static_pointer_cast<CDVDMsgPlayerSetAudioStream>(pMsg);
    ...
    {
      std::shared_ptr<CDVDMsgPlayerSetVideoStream> pMsg2 = std::static_pointer_cast<CDVDMsgPlayerSetVideoStream>(pMsg);
    ...
    {
      std::shared_ptr<CDVDMsgPlayerSetSubtitleStream> pMsg2 = std::static_pointer_cast<CDVDMsgPlayerSetSubtitleStream>(pMsg);
    ...
    {
      std::shared_ptr<CDVDMsgBool> pValue = std::static_pointer_cast<CDVDMsgBool>(pMsg);
      SetSubtitleVisibleInternal(pValue->m_value);
    }
    else if (pMsg->IsType(CDVDMsg::PLAYER_SET_PROGRAM))
    {
      std::shared_ptr<CDVDMsgInt> msg = std::static_pointer_cast<CDVDMsgInt>(pMsg);
      if (m_pDemuxer)
      {
    ...
    else if (pMsg->IsType(CDVDMsg::PLAYER_SET_STATE))
    {
      ...
      std::shared_ptr<CDVDMsgPlayerSetState> pMsgPlayerSetState = std::static_pointer_cast<CDVDMsgPlayerSetState>(pMsg);
...
void CVideoPlayer::SetPlaySpeed(int speed)
{
  if (IsPlaying())
  {
    ...
    m_messenger.Put(std::shared_ptr<CDVDMsgPlayerSetSpeed>(new CDVDMsgPlayerSetSpeed(params)));
  }
  else
  {
    m_playSpeed = speed;
...
{
  m_messenger.Put(std::shared_ptr<CDVDMsgBool>(new CDVDMsgBool(CDVDMsg::PLAYER_SET_SUBTITLESTREAM_VISIBLE, bVisible)));
...
void CVideoPlayer::FrameAdvance(int frames)
{
  ...
  m_messenger.Put(std::shared_ptr<CDVDMsgInt>(new CDVDMsgInt(CDVDMsg::PLAYER_FRAME_ADVANCE, frames)));
}
...
bool CVideoPlayer::OpenAudioStream(CDVDStreamInfo& hint, bool reset)
{
     ...
     m_CurrentAudio.hint != hint)
  {
    ...
    player->SendMessage(std::shared_ptr<CDVDMsg>(new CDVDMsg(CDVDMsg::GENERAL_RESET)), 0);

  m_HasAudio = true;

  static_cast<IDVDStreamPlayerAudio*>(player)->SendMessage(std::shared_ptr<CDVDMsg>(new CDVDMsg(CDVDMsg::PLAYER_REQUEST_STATE)), 1);
...
bool CVideoPlayer::OpenVideoStream(CDVDStreamInfo& hint, bool reset)
{
  ...
  static_cast<IDVDStreamPlayerVideo*>(player)->SendMessage(std::shared_ptr<CDVDMsg>(new CDVDMsg(CDVDMsg::PLAYER_REQUEST_STATE)), 1);
...
void CVideoPlayer::FlushBuffers(double pts, bool accurate, bool sync)
{
      ...
      m_playSpeed == DVD_PLAYSPEED_PAUSE)
  {
    ...
    std::shared_ptr<CDVDMsgGeneralSynchronize> msg(new CDVDMsgGeneralSynchronize(1000, SYNCSOURCE_AUDIO | SYNCSOURCE_VIDEO));
...
int CVideoPlayer::OnDiscNavResult(void* pData, int iMessage)
{
  ...
  if (m_pInputStream->IsStreamType(DVDSTREAM_TYPE_BLURAY))
  {
    ...
    case BD_EVENT_PLAYLIST_STOP:
      m_messenger.Put(std::shared_ptr<CDVDMsg>(new CDVDMsg(CDVDMsg::GENERAL_FLUSH)));
      break;
  ...
  if (m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
  {
    ...
    case DVDNAV_SPU_CLUT_CHANGE:
      {
        m_VideoPlayerSubtitle->SendMessage(std::shared_ptr<CDVDMsgSubtitleClutChange>(new CDVDMsgSubtitleClutChange((uint8_t*)pData)));
      }
      break;
    ...
    case DVDNAV_VTS_CHANGE:
      {
        ...
        if( m_VideoPlayerVideo->IsInited() )
          m_VideoPlayerVideo->SendMessage(std::shared_ptr<CDVDMsgDouble>(new CDVDMsgDouble(CDVDMsg::VIDEO_SET_ASPECT, m_CurrentVideo.hint.aspect)));
...
#define THREAD_ACTION(action) \
  do { \
    if (!IsCurrentThread()) { \
      m_messenger.Put(std::shared_ptr<CDVDMsgType<CAction>>(new CDVDMsgType<CAction>(CDVDMsg::GENERAL_GUI_ACTION, action))); \
      return true; \
    } \
  } while(false)
...
/* this code is disabled to allow switching playlist items (dvdimage "stacks") */
  ...
  switch (action.GetID())
  {
    case ACTION_NEXT_ITEM:
      ...
      {
        m_messenger.Put(std::shared_ptr<CDVDMsgPlayerSeekChapter>(new CDVDMsgPlayerSeekChapter(GetChapter() + 1)));
        g_infoManager.SetDisplayAfterSeek();
        return true;
      }
...
{
  m_messenger.Put(std::shared_ptr<CDVDMsgPlayerSetState>(new CDVDMsgPlayerSetState(state)));
  return true;
}
...
int CVideoPlayer::SeekChapter(int iChapter)
{
  if (GetChapter() > 0)
  {
    ...
    // Seek to the chapter.
    m_messenger.Put(std::shared_ptr<CDVDMsgPlayerSeekChapter>(new CDVDMsgPlayerSeekChapter(iChapter)));
    SynchronizeDemuxer();
  }
...
{
  m_messenger.Put(std::shared_ptr<CDVDMsgType<std::string>>(new CDVDMsgType<std::string>(CDVDMsg::SUBTITLE_ADDFILE, strSubPath)));
}
...
{
  m_messenger.Put(std::shared_ptr<CDVDMsg>(new CDVDMsg(CDVDMsg::PLAYER_AVCHANGE)));
}
...
void CVideoPlayer::OnLostDisplay()
{
  ...
  m_VideoPlayerAudio->SendMessage(std::shared_ptr<CDVDMsgBool>(new CDVDMsgBool(CDVDMsg::GENERAL_PAUSE, true)), 1);
  m_VideoPlayerVideo->SendMessage(std::shared_ptr<CDVDMsgBool>(new CDVDMsgBool(CDVDMsg::GENERAL_PAUSE, true)), 1);
  m_clock.Pause(true);
  m_displayLost = true;
}
...
void CVideoPlayer::OnResetDisplay()
{
  ...
  m_VideoPlayerAudio->SendMessage(std::shared_ptr<CDVDMsgBool>(new CDVDMsgBool(CDVDMsg::GENERAL_PAUSE, false)), 1);
  m_VideoPlayerVideo->SendMessage(std::shared_ptr<CDVDMsgBool>(new CDVDMsgBool(CDVDMsg::GENERAL_PAUSE, false)), 1);
  m_clock.Pause(false);
  m_displayLost = false;
}
...
{
  m_messenger.Put(std::shared_ptr<CDVDMsgPlayerSetVideoStream>(new CDVDMsgPlayerSetVideoStream(iStream)));
...
{
  m_messenger.Put(std::shared_ptr<CDVDMsgPlayerSetAudioStream>(new CDVDMsgPlayerSetAudioStream(iStream)));
...
{
  m_messenger.Put(std::shared_ptr<CDVDMsgPlayerSetSubtitleStream>(new CDVDMsgPlayerSetSubtitleStream(iStream)));
...
{
  m_messenger.Put(std::shared_ptr<CDVDMsgInt>(new CDVDMsgInt(CDVDMsg::PLAYER_SET_PROGRAM, progId)));
}
