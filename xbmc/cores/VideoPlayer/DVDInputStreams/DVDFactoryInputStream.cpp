...
std::unique_ptr<CDVDInputStream> CDVDFactoryInputStream::CreateInputStream(IVideoPlayer* pPlayer, const CFileItem &fileitem, bool scanforextaudio)
{
  ...
  for (auto addonInfo : addonInfos)
  {
    if (CInputStreamAddon::Supports(addonInfo, fileitem))
      return std::unique_ptr<CInputStreamAddon>(new CInputStreamAddon(addonInfo, pPlayer, fileitem));
  }
  ...
  if (fileitem.IsDiscImage())
  {
    ...
    if(XFILE::CFile::Exists(url.Get()))
      return std::unique_ptr<CDVDInputStreamBluray>(new CDVDInputStreamBluray(pPlayer, fileitem));
#endif

    return std::unique_ptr<CDVDInputStreamNavigator>(new CDVDInputStreamNavigator(pPlayer, fileitem));
  }
  ...
  if (fileitem.IsDVDFile(false, true))
    ...
    return std::unique_ptr<CDVDInputStreamPVRManager>(new CDVDInputStreamPVRManager(pPlayer, fileitem));
#ifdef HAVE_LIBBLURAY
    ...
    return std::unique_ptr<CDVDInputStreamFFmpeg>(new CDVDInputStreamFFmpeg(fileitem));
#ifdef ENABLE_DVDINPUTSTREAM_STACK
  else if(file.substr(0, 8) == "stack://")
    return std::unique_ptr<CDVDInputStreamStack>(new CDVDInputStreamStack(fileitem));
#endif
  ...
  if (finalFileitem.IsInternetStream())
  {
    ...
    if (finalFileitem.IsType(".m3u8"))
      return std::unique_ptr<CDVDInputStreamFFmpeg>(new CDVDInputStreamFFmpeg(finalFileitem));
  ...
  return std::unique_ptr<CDVDInputStreamFile>(new CDVDInputStreamFile(finalFileitem));
}

std::unique_ptr<CDVDInputStream> CDVDFactoryInputStream::CreateInputStream(IVideoPlayer* pPlayer, const CFileItem &fileitem, const std::vector<std::string>& filenames)
{
  return std::unique_ptr<CInputStreamMultiSource>(new CInputStreamMultiSource(pPlayer, fileitem, filenames));
}
