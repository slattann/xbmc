...
void CGUIDialogNumeric::FrameMove()
{
  ...
  { // format up the time
    strLabel = StringUtils::Format("%2d:%02d:%02d", m_datetime.hour, m_datetime.minute, m_datetime.second);
    start = m_block * 3;
    end = m_block * 3 + 2;
  }
  else if (m_mode == INPUT_DATE)
  { // format up the date
    strLabel = StringUtils::Format("%2d/%2d/%4d", m_datetime.day, m_datetime.month, m_datetime.year);
    start = m_block * 3;
    end = m_block * 3 + 2;
    if (m_block == 2)
      end = m_block * 3 + 4;
...
std::string CGUIDialogNumeric::GetOutputString() const
{
  ...
  case INPUT_TIME_SECONDS:
    return StringUtils::Format("%i:%02i:%02i", m_datetime.hour, m_datetime.minute, m_datetime.second);
  case INPUT_IP_ADDRESS:
...
void CGUIDialogNumeric::VerifyDate(bool checkYear)
{
  ...
  {
    if (m_datetime.month == 4 || m_datetime.month == 6 || m_datetime.month == 9 || m_datetime.month == 11)
      m_datetime.day = 30;
  }
